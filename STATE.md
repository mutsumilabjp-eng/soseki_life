...
- 2026-09-11: furusatoアカウントから専用の`soseki_life` Cloudflareアカウント（account_id `8cd7ecdaf8eb44bf0d12bd406fa9d0dd`、workers.dev共通名`soseki-life`）へ実体を移行。新規D1 `natsume-free-sheet-events`（uuid `0e39f606-3a5e-4743-a45d-654435f88a8c`）を作成しマイグレーション適用、Workerを`https://natsume.soseki-life.workers.dev`にデプロイ。`/`・`/shigoto-seiri/`・`/privacy/`とも200を確認済み（デプロイ直後は新規workers.devサブドメインのDNS伝播待ちで一時404、15秒程度で解消）。`wrangler.toml`のaccount_id/database_idを更新しコミット、GitHub `mutsumilabjp-eng/soseki_life`（新規・空リポジトリ）へ初回push済み。furusatoアカウント側の旧D1（uuid `1e6254ec-...`）と、furusato上に残った空の紛れWorker`natsume`（実データを持つ本体は`natsume-free-sheet`という別名Worker）は未削除のまま——需要不要と確定したら人間の判断でクリーンアップする。プロフィールリンク等の外部公開更新は未実施。
- 2026-09-11: ユーザー指示により**公開URLはルート`https://natsume.soseki-life.workers.dev/`を正**とする（`/shigoto-seiri/`はレガシーパスとして引き続き同じ内容を表示するのみ）。コード側は元々`/`が同じワークシート全文を返す構成のため変更不要。README「プロフィールに設定するリンク」は既に`<公開ドメイン>/?utm_source=...`とルート基準で記載済み。今後この案件のURLを案内する時はルートを使う。
